...
VAR
  ...
  long rx, ry, rz, temp, ax, ay, az, arx, ary ,ax0, ay0, az0
...
PRI FindZero | tc, xc, yc, zc, xa, ya, za, dr
...
'  -------------------------------------------------------------- 
        ...
        ''''''''
        add             p1, #4
        mov             paAX0, p1                        ' Store the pointer to the aX0 var in HUB RAM
        add             p1, #4
        mov             paAY0, p1                        ' Store the pointer to the aY0 var in HUB RAM
        add             p1, #4
        mov             paAZ0, p1                        ' Store the pointer to the aZ0 var in HUB RAM
...
'------------------------------------------------------------------------------
ITGReadValues           'mov     i2cDevID, #%11010010    ' Device ID of the ITG3200
                        'Change to Master mode
                        'mov     i2cDevID, #%11010000     'Device ID for the IMU3000
                        'mov     i2cAddr, #61
                        'mov     i2cValue, #$28             'AUX_IF_EN=1 | AUX_IF_RST =1
...
ADXLReadValues
                        ...
                        'Change to Pass Through mode
                        'mov     i2cDevID, #%11010000     'Device ID for the IMU3000
                        'mov     i2cAddr, #61
                        'mov     i2cValue, #$08             'AUX_IF_EN=0 | AUX_IF_RST =1
                        'call    #i2cWriteRegisterByte
...
'------------------------------------------------------------------------------
ADXLComputeAngles       rdlong  iaZ0, paAZ0
                        subs    iaZ, iaZ0 
                        mov     cx, iaZ
                        rdlong  iaX0, paAX0 
                        subs    iaX, iaX0 
...
:ITGSetConfig           'mov     i2cDevID, #%11010010     'Device ID for the ITG3200
                        mov      i2cDevID, #%11010000     'Device ID for the IMU3000
                        'Me aseguro que este en modo Master
                        mov     i2cAddr, #61
                        mov     i2cValue, #$28             'AUX_IF_EN=1 | AUX_IF_RST =1
                        call    #i2cWriteRegisterByte
                        
                        'Prueba con modo Pass through desde el comienzo
                        mov     i2cAddr, #61
                        mov     i2cValue, #$08             'AUX_IF_EN=0 | AUX_IF_RST =1 
                        call    #i2cWriteRegisterByte
                        
                        mov     i2cAddr, #22
                        mov     i2cValue, #$18         'DLPF_FS (DLPF_CFG = 0, FS_SEL = 3) = 8Khz sample, 256Hz lowpass
                        call    #i2cWriteRegisterByte

                        mov     i2cAddr, #21
                        mov     i2cValue, #3             'SMPLRT_DIV = 3(+1) = 8KHz / 4 = 2KHz sample rate
                        call    #i2cWriteRegisterByte
                        ...
                        'Step 4 Aux Addr
                        mov     i2cAddr, #20
                        mov     i2cValue, #$53             '7bit Address for ADXL345
                        call    #i2cWriteRegisterByte

                        'Step 5 Aux Burst Addr
                        mov     i2cAddr, #24
                        mov     i2cValue, #$32             'ADXL_DATA_X0 Addr
                        call    #i2cWriteRegisterByte
...
:ADXLSetConfig          mov     i2cDevID, #%10100110    'Device ID for the ADXL345  0xA6(Sdo to GND)
                        mov     i2cAddr, #$31
                        mov     i2cValue, #%1011        'Set full range mode, LSB justify, 16g range
                        call    #i2cWriteRegisterByte
                        
                        'El registro correcto es el 0x2C
                        'Segun Jason el sample rate debe ser 1600 then #%1110
                        mov     i2cAddr, #$2C
                        ...
                        'Volver de modo pass through a modo Master
                        'mov     i2cDevID, #%11010000     'Device ID for the IMU3000
                        'mov     i2cAddr, #61
                        'mov     i2cValue, #$28             'AUX_IF_EN=1 | AUX_IF_RST =1
                        'call    #i2cWriteRegisterByte
...
' agregado x nosotros
paAX0                   long    0                       ' Pointer to X accel BIAS in hub ram
paAY0                   long    0                       ' Pointer to Y accel BIAS in hub ram
paAZ0                   long    0                       ' Pointer to Z accel BIAS in hub ram
'
